...
/*
 * The original author of this file is unknown.
 */
...
uint8_t
enc28j60_read_op( uint8_t cmd, uint8_t adr )
{
    uint8_t data;
...
void
enc28j60_write_op( uint8_t cmd, uint8_t adr, uint8_t data )
{
    enc28j60_select( );
    enc28j60_tx( cmd | ( adr & ENC28J60_ADDR_MASK ) );
    enc28j60_tx( data );
...
// Write register
void
enc28j60_wcr( uint8_t adr, uint8_t arg )
{
    enc28j60_set_bank( adr );
...
// Write register pair
void
enc28j60_wcr16( uint8_t adr, uint16_t arg )
{
    enc28j60_set_bank( adr );
    enc28j60_write_op( ENC28J60_SPI_WCR, adr, arg );
...
void
enc28j60_bfc( uint8_t adr, uint8_t mask )
{
    enc28j60_set_bank( adr );
...
void
enc28j60_bfs( uint8_t adr, uint8_t mask )
{
    enc28j60_set_bank( adr );
...
void
enc28j60_read_buffer( uint8_t* buf, uint16_t len )
{
    enc28j60_select( );
...
void
enc28j60_write_buffer( uint8_t* buf, uint16_t len )
{
    enc28j60_select( );
...
// Write PHY register
void
enc28j60_write_phy( uint8_t adr, uint16_t data )
{
    enc28j60_wcr( MIREGADR, adr );
...
/*
 * Init and send/receive packet
 */
void
enc28j60_init( const uint8_t* macadr )
{
    // Initialize SPI
    ...
    enc28j60_write_phy( PHLCON, PHLCON_LACFG2 |   // Configure LED ctrl
                                    PHLCON_LBCFG2 | PHLCON_LBCFG1 | PHLCON_LBCFG0 | PHLCON_LFRQ0
                                    | PHLCON_STRCH );
...
void
enc28j60_send_packet( uint8_t* data, uint16_t len )
{
...
uint16_t
enc28j60_recv_packet( uint8_t* buf, uint16_t buflen )
{
